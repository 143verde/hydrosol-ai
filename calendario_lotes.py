...
with tab3:
    ...
    for _, lote in df_lotes.iterrows():
        fig_prog.add_trace(go.Indicator(
            ...
            title={'text': lote['nome'][:20] + "..."},
            gauge={
                'axis': {'range': [0, 100]},
                'bar': {'color': cores_status.get(lote['status'], '#667eea')},
                'steps': [
            ...
            domain={'row': 0, 'column': list(df_lotes.index).index(_)}
        ))

    fig_prog.update_layout(
        grid={'rows': 1, 'columns': len(df_lotes), 'pattern': "independent"},
        height=250
    )
